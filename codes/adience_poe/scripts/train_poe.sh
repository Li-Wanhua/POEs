...
--batch-size=32 \
--test-batch-size=32 \
--max-epochs=50 --lr-decay-epoch=30 --lr=0.0001 --fc-lr=0.0001 \
...
--test-data-file=./data_list/test_fold_is_${fold}/age_val.txt \
--num-workers=4 --distance='JDistance' \
--alpha-coeff=1e-4 --beta-coeff=1e-4 --margin=5 \
--exp-name=train_val_fold_${fold} \
--logdir=./log/adience_${main_loss_type} \
> ./log/adience_${main_loss_type}_train_val_fold_${fold}.txt 2>&1
done
